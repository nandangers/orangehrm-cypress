...
describe("OrangeHRM - Login Feature (Intercept)", () => {
  ...
  // ─── Endpoint API ─────────────────────────────────────────
  const API = {
    // FIX: OrangeHRM memanggil API messages ini saat halaman login di-load
  ...
  it("TC-01 | Elemen UI halaman login tampil dengan benar", () => {
    ...
    // ✅ Intercept: akan berhasil karena API i18n/messages memang dipanggil
    cy.wait("@pageLoadAPI", { timeout: 15000 }).then((interception) => {
      // Terkadang API i18n mengembalikan 304 (Not Modified) jika di-cache browser
